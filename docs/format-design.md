...
At 167 MiB it is bigger than the Parquet output and much bigger than the CSV
...
data/raw/**.xlsx                  364 files, 30.4 MiB   immutable, committed
  ...
  +-- data/processed/solardata.db   167 MiB   not committable (>100 MiB)
...
clone without running anything. `solardata.db` is not, because at 167 MiB it
...
`data/raw` is 30.4 MiB and `solardata.db` is 166.7 MiB, which looks alarming
